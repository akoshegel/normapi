...
class MongoConnection {
	...
	addSchema(key, schema, collection) {
		if (this._schemas.hasOwnProperty(key)) {
			throw new Error(`Duplicated schema error - MongoConnection at location '${this._location}'`);
		} else {
			this._schemas[key] = this._connection.model(key, schema, collection);
		}
	}
}
